...
#### Write through
1. **j = 0 ~ 3, j = 4 ~ 7 都會個做以下步驟一次（總共做兩次）：**
	* 發生cache miss，一個block從memory --> L2 cache --> L1 cache，中間經過 ***2次*** 的傳輸
	* CPU --> L1 Cache：CPU把4 bytes的資料寫到L1 Cache中，這裡做了 **4次**
	* L1 --> L2 --> Memory，這圖中做了兩次 data trasferring，每一次transfer都會消耗時間，所以這個階段經過 ***2次*** Transfer。不過一個區間( ***j = 0 -> 3*** or ***j = 4 -> 7*** )做了 ***4次*** write，所以這邊總共做了**4 x 2 = 8次**
	* block傳輸總共有 **2 + 8 = 10 次**，**4 bytes 傳輸有 4次**

2. 以上的步驟在 **j = 0 ~ 3, j = 4 ~ 7 都會個做以下步驟一次（總共做兩次)** ，所以block傳輸總共有 **2 x 10 = 20 次**，**4 bytes 有 2 x 4 = 8 次**，那麼總傳輸時間：

##### <center> $Total\ transfering\ cycles\\ \ \ = 20\ transferr \times \left( 10 + 5 \times \left( \lceil \frac{block\ size}{8} \rceil - 1 \right) \right) + 8\ transfer \times (10 + 5 \times (\lceil \frac{4}{8} \rceil - 1)) \\ \ \ = 20 \times (10 + 5 \times ( \lceil \frac{32}{8} \rceil - 1 ) ) + 80\\ \ \ = 20 \times 25 + 80 = 580\ cycles$ </center>
