...
if [[ "$ARGC" != 6 ]]; then # Bad number of arguments.
   ...
   echo ""
    ...
    echo "EXAMPLE: ./run_gem5_alpha_spec06_benchmark.sh bzip2 /FULL/PATH/TO/output_dir D_CACHE_SIZE I_CACHE_SIZE TOTAL L2_CACHE_SIZE"
    echo ""
...
echo "D_cache = $D_CACHE, I_CACHE = $I_CACHE, L2 = $L2_CACHE"
...
OUTPUT=_${L2_CACHE}
$GEM5_DIR/build/ALPHA_MOESI_CMP_directory/gem5.fast --outdir=$OUTPUT_DIR $GEM5_DIR/configs/example/spec06_config.py --cpu-type=timing --cpu-clock "2GHz" --mem-size="1024MB" --caches --l2cache --l1d_size=$D_CACHE --l1i_size=$I_CACHE --l2_size=$L2_CACHE --benchmark=$BENCHMARK --benchmark_stdout=$OUTPUT_DIR/${OUTPUT}.out --benchmark_stderr=$OUTPUT_DIR/${OUTPUT}.err  | tee -a $SCRIPT_OUT

cd -

mv out/stats.txt out/stats_${L2_CACHE}.txt
mv out/config.json out/config_${L2_CACHE}.json
mv out/config.ini out/config_${L2_CACHE}.ini
mv out/runscript.log out/runscript_${L2_CACHE}.log

cd ./out
files=$(ls | grep -i "_${L2_CACHE}")
folder=${L2_CACHE}
mkdir -p ./$folder
mv $files ./$folder
