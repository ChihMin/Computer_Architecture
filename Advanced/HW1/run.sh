...
GEM5_DIR=/home/chihmin/gem5                     # Install location of gem5
SPEC_DIR=/home/chihmin/gem5/GEM5_SPEC2006                  # Install location of your SPEC2006 benchmarks
##################################################################
...
if [[ "$ARGC" != 5 ]]; then # Bad number of arguments.
   ...
   echo ""
    ...
    echo "EXAMPLE: ./run_gem5_alpha_spec06_benchmark.sh bzip2 /FULL/PATH/TO/output_dir D_CACHE_SIZE I_CACHE_SIZE TOTAL"
    echo ""
...
echo "D_cache = $D_CACHE, I_CACHE = $I_CACHE"
...
RUN_DIR=$SPEC_DIR/bin     # Run directory for the selected SPEC benchmark
...
OUTPUT=${BENCHMARK}_${D_CACHE}_${I_CACHE}
$GEM5_DIR/build/ALPHA_MOESI_CMP_directory/gem5.fast --outdir=$OUTPUT_DIR $GEM5_DIR/configs/example/spec06_config.py --cpu-type=timing --cpu-clock "2GHz" --mem-size="1024MB" --caches --l1d_size=$D_CACHE --l1i_size=$I_CACHE  --benchmark=$BENCHMARK --benchmark_stdout=$OUTPUT_DIR/${OUTPUT}.out --benchmark_stderr=$OUTPUT_DIR/${OUTPUT}.err  | tee -a $SCRIPT_OUT

cd -
mv out/stats.txt out/stats_${D_CACHE}_${I_CACHE}.txt
mv out/config.json out/config_${D_CACHE}_${I_CACHE}.json
mv out/config.ini out/config_${D_CACHE}_${I_CACHE}.ini
mv out/runscript.log out/runscript_${D_CACHE}_${I_CACHE}.log

cd ./out
files=$(ls | grep -i "_${D_CACHE}_${I_CACHE}")
folder=${D_CACHE}_${I_CACHE}
mkdir -p ./$TOTAL
mkdir -p ./$TOTAL/$folder
mkdir -p ./$TOTAL/$folder/$BENCHMARK
mv $files ./$TOTAL/$folder/$BENCHMARK
